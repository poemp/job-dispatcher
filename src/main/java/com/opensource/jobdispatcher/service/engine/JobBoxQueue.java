...
import com.opensource.jobdispatcher.service.alert.AlertService;
			...
			if(null!=tasksCanExecute) {
				...
				for(Task task:tasksCanExecute) {
					...
					if(retryMap.get(taskCode)>0) {
					    ...
					    AlertService alertService = SpringTool.getBean(AlertService.class);
					    alertService.alert(jobCode, content);
